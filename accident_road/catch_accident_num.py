...
grouped_df1 = merged_df.groupby(['RoadSection', 'Class']).size().reset_index(name='事故數量')
grouped_df2 = merged_df.groupby(['RoadSection', 'Direction']).size().reset_index(name='事故數量')

# 使用 pivot_table 將事故類別作為列，路段作為索引，並填充每個單元格的值為對應事故類型的數量
pivot_table_df1 = grouped_df1.pivot_table(index='RoadSection', columns='Class', values='事故數量', fill_value=0, aggfunc='first')
pivot_table_df2 = grouped_df2.pivot_table(index='RoadSection', columns='Direction', values='事故數量', fill_value=0,aggfunc='first')
...
# 保存結果到新的 CSV 文件
merged_pivot_table_df.to_csv("accident_count2.csv",encoding='utf-8-sig')

print("結果已保存到 accident_count2.csv 文件中。")
